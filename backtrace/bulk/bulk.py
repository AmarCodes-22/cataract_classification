    ...
    try:
        ...
        if image_category == "fail":
            ...
            if not image_already_processed(
                image_name, directories["fail_images_dir"], directories["ai_dash_dir"], directories["manual_dir"]
            ):
                if not os.path.exists(image_path):
            ...
            without_backtrace_exists = os.path.exists(
                os.path.join(directories["without_backtrace_dir"], image_name + ".jpg")
            )
            ...
            if not backtrace_exists or not without_backtrace_exists:
                if not backtrace_exists:
                    ...
                    backtrace_payload.update({"force_process": {"auto-segmentation-interior_removebg": "backtrace"}})
                ...
                if not without_backtrace_exists:
                    ...
                    without_backtrace_payload.update({"force_process": None})
                    ...
                    if without_backtrace_data and "url" in without_backtrace_data:
                        save_output_image(
                            without_backtrace_data["url"], directories["without_backtrace_dir"], image_name
                        )
